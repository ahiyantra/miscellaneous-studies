...
def text_similarity(query, document):
    ...
    sum_val = 0
    N = sum(doc_freq.values())  # Total number of words in the document (including stop-words)
    ...
    print(f"sim(q, d) =>> {sum_val}/{N} = {sim}")  # Restored debug message
    return sim
...
def main():
    ...
    for i, doc in enumerate(DOCS):
        sim = text_similarity(QUERY, doc)
        similarities.append(sim)
        print(f"Similarity with Document {chr(65 + i)}: {sim:.5f}")
    ...
    SIM1_A = similarities[0] + 0.5 * PR_A
    SIM1_B = similarities[1] + 0.5 * PR_B
    SIM1_C = similarities[2] + 0.5 * PR_C
    SIM1_D = similarities[3] + 0.5 * PR_D
    SIM1_E = similarities[4] + 0.5 * PR_E
    ...
    SIM2_A = similarities[0] + 0.5 * new_PR_A
    SIM2_B = similarities[1] + 0.5 * new_PR_B
    SIM2_C = similarities[2] + 0.5 * new_PR_C
    SIM2_D = similarities[3] + 0.5 * new_PR_D
    SIM2_E = similarities[4] + 0.5 * new_PR_E
